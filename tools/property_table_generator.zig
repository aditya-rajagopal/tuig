...
pub fn generatePropertyLookupTable2Stage(
    io: std.Io,
    ...
    var stage2: std.ArrayList(Property) = .empty;

    const block_size = 256;
    const Block = [block_size]Property;
    ...
    var block_map = std.HashMap(Block, u16, struct {
        ...
        pub fn eql(_: @This(), a: Block, b: Block) bool {
            for (a, b) |ai, bi| {
                if (@as(u8, @bitCast(ai)) != @as(u8, @bitCast(bi))) return false;
            }
            return true;
        }
    ...
    for (0..std.math.maxInt(u21)) |cp| {
        ...
        if (index_block < block_size) @memset(block[index_block..block_size], .invalid);
    ...
    try writer.writeAll(
        ...
        \\    return property_table.stage2[index_stage2];
        \\}
        \\
        \\
    );
    ...
    try writer.print(
        \\    pub const stage2: [{d}]Property align(std.atomic.cache_line) = .{{
        \\
    , .{stage2.items.len});
...
pub fn generatePropertyLookupTable3Stage(
    io: std.Io,
    allocator: std.mem.Allocator,
    file_path: []const u8,
    info: UnicodeInfo,
